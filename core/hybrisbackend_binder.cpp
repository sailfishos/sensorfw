...
HybrisBackendBinder::HybrisBackendBinder(HybrisManager *manager)
...
void HybrisBackendBinder::cleanup()
{
    ...
    m_serviceManager = nullptr;
    m_remote = nullptr; // auto-release

    for (int i = 0 ; i < m_sensorCount ; i++) {
        g_free((char *)m_sensorArray[i].name.data.str);
        g_free((char *)m_sensorArray[i].vendor.data.str);
        g_free((char *)m_sensorArray[i].typeAsString.data.str);
        g_free((char *)m_sensorArray[i].requiredPermission.data.str);
    }
...
bool HybrisBackendBinder::needsWakeup(const sensors_event_t *eve)
{
    ...
    return (sensor->flags & SENSOR_FLAG_WAKE_UP);
}
